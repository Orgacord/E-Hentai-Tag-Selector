            ...
            if (file) {
                ...
                reader.onload = function (event) {
                    try {
                        ...
                        // Restore pinned tags
                        localStorage.setItem("pinnedTags", JSON.stringify(config.pinnedTags));
                        config.pinnedTags.forEach(tag => {
                            const [category, ...tagParts] = tag.split(":");
                            const tagName = tagParts.join(":");
                            const label = document.querySelector(`label[data-category="${category}"][data-tag="${tagName}"]`);
                            if (label) {
                                const pinIcon = label.querySelector(".pin-icon");
                                if (pinIcon) {
